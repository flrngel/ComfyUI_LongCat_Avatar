...
def load_quantized_dit(checkpoint_dir: str, subfolder: str = "base_model_int8", single_file =None, **kwargs):
    ...
    with torch.device('meta'):
        #model = LongCatVideoAvatarTransformer3DModel(**config).to(torch.bfloat16)  # Start with float32 for weight loading
        model = LongCatVideoAvatarTransformer3DModel(**config)# Start with float32 for weight loading
    ...
    if os.path.exists(index_path) and single_file is None:
    ...
    print("Load state dict result:", X)
