...
class Wav2Vec2ModelWrapper(nn.Module):
    def __init__(self, config_path, device='cuda', prefix='wav2vec2.',model_path=None):
        ...
        model_path = model_path or os.path.join(config_path, 'pytorch_model.bin')
        state_dict = torch.load(model_path, map_location=device, weights_only=True) if model_path.endswith('.bin') else safe_load_file(model_path, )
