...
def _attn_fwd_bsa_varlen_align(
    ...
    # load scales
    qk_scale = sm_scale
    qk_scale *= 1.44269504  # 1/ln2; exp2(x/ln2) == exp2(ln(e^x) / ln2) == exp2(log2(e^x)) == exp(x); after multiplying by 1/ln2, exp2(x/ln2) == exp(x), and exp2 is faster
    # load q: it will stay in SRAM throughout
    q = tl.load(Q_block_ptr)
    S = tl.load(block_indices_lens)
    for i in range(S):
        ...
        # update acc
        v = tl.load(V_block_ptr_i)
        acc = tl.dot(p.to(v.dtype), v, acc) # not divided by se, an optimization introduced by fa2
        # update m_i and l_i
        # place this at the end of the loop to reduce register pressure: https://github.com/triton-lang/triton/commit/ee6abd9
        l_i = l_i * alpha + l_ij # current total se
        m_i = m_ij
