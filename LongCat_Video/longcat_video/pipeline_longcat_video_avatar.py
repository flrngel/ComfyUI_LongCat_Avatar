...
def get_audio_embedding_whisper_(audio_encoder,speech_array, fps=25, device='cpu', sample_rate=16000):
    """Extract audio features using the Whisper encoder.
    Args:
        speech_array: raw audio waveform (numpy array, mono, sample_rate=16000)
    ...
    def linear_interpolation_fps(features, input_fps, output_fps, output_len=None):
        ...
        Args:
            ...
            output_len: if specified, use this length directly; otherwise compute from the fps ratio
        Returns:
            [B, output_len, D]
        """
    ...
    # ---- Constants ----
    MEL_CHUNK      = 750 * 640   # feature extractor sliding window size (number of samples)
    ENC_CHUNK      = 3000        # encoder sliding window size (mel frames)
    ENC_FPS        = 50          # encoder output frame rate
    ...
    # ---- Group by layer + interpolate to target frame count ----
...
def get_audio_embedding_whisper(audio_encoder,audio_feature_extractor,speech_array, fps=25, device='cpu', sample_rate=16000):
    ...
    audio_prompts = audio_prompts[:, :video_length * 2]  # trim to valid frames
...
class LongCatVideoAvatarPipeline:
    ...
    def __init__(
        ...
        self.streaming_prefetch_count = None
    ...
    def _get_prompt_embeds_masks( self,text ,device,max_sequence_length=512 ):
    ...
    def _cache_clean_latents(self, cond_latents, model_max_length, offload_kv_cache, device, dtype, audio_embs, num_cond_latents, num_ref_latents, ref_img_index):
        ...
        model_context = self._model_ctx( self.streaming_prefetch_count) if self.streaming_prefetch_count is not None else self.dit
        with model_context as self.dit:
    ...
    def generate_at2v(
        ...
        prompt_attention_mask=self._get_prompt_embeds_masks( text[0] ,device, )
        negative_prompt_attention_mask=self._get_prompt_embeds_masks(text[1] ,device, )
        ...
        #         context_parallel_util.cp_broadcast(negative_prompt_embeds)
        #         context_parallel_util.cp_broadcast(negative_prompt_attention_mask)
        print("text", prompt_embeds.shape, negative_prompt_embeds.shape)
        print("audio", audio_emb.shape)
